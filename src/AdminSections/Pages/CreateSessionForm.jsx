...
const FACULTIES_DATA = {
  ...
  const getAdminProfileFromToken = () => {
    try {
      ...
      return JSON.parse(atob(payloadBase64)) || null;
    } catch {
      return null;
    }
  };
  ...
  const [formData, setFormData] = useState({
    ...
    academicLevel: adminProfile?.level || "",
  });
  ...
  useEffect(() => {
    ...
    setFormData((prev) => ({
      ...
      dateTimeFrom: getFormattedLocalDateTime(0),
      dateTimeTo: getFormattedLocalDateTime(1),
      faculty: prev.faculty || profile?.faculty || "",
      department: prev.department || profile?.department || "",
      academicLevel: prev.academicLevel || profile?.level || "",
    }));
  }, []);
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div>
              ...
              >
                ...
                {formData.academicLevel && !["100L", "200L", "300L", "400L", "500L", "100", "200", "300", "400", "500"].includes(formData.academicLevel) && (
            ...
            {/* Profile Auto-Fill Indicator Banner */}
            {adminProfile?.faculty && (
              <div className="col-span-1 md:col-span-2 bg-[#e8f0ec] border border-[#0a643a]/20 rounded-xl p-3.5 mb-2 flex items-center gap-3">
                <span className="material-symbols-outlined text-[#0a643a]">verified_user</span>
                <div className="text-xs text-[#0a643a]">
                  <strong className="font-bold">Auto-filled Context:</strong> {adminProfile.role === 'super_admin' ? 'Faculty Super Admin' : adminProfile.role === 'course_rep' ? 'Course Representative' : 'Department Admin'} ({adminProfile.faculty} {adminProfile.department ? `• ${adminProfile.department}` : ''} {adminProfile.level ? `• ${adminProfile.level}` : ''})
                </div>
              </div>
            )}
            ...
            <div>
              ...
              >
                ...
                {formData.faculty && !Object.keys(FACULTIES_DATA).includes(formData.faculty) && (
            ...
            <div>
              ...
              >
                ...
                {formData.department && (!FACULTIES_DATA[formData.faculty] || !FACULTIES_DATA[formData.faculty].includes(formData.department)) && (
                  <option value={formData.department}>{formData.department}</option>
                )}
                {formData.faculty && FACULTIES_DATA[formData.faculty] &&
