...
const AttendanceHistory = () => {
    ...
    const fetchStudentAttendance = async (pageNumber = 1) => {
        if (!token) return
        setLoading(true)
        try {
            // Include page, limit, and optional course filter in request
            let url = `https://smart-backend-1-q3fb.onrender.com/my-attendance?page=${pageNumber}&limit=5`
            if (selectedCourse) {
                url += `&courseCode=${encodeURIComponent(selectedCourse)}`
            }
                ...
                <div className='py-4 px-4 sm:px-6 lg:px-10 w-full pb-28 mt-[4rem] lg:mt-[0rem]'>
                    <div className='flex flex-col'>
                        ...
                        <div className='flex flex-col lg:flex-row lg:items-center justify-between gap-4'>
                            <div>
                                <h1 className='text-[24px] sm:text-[30px] font-bold'>Attendance History</h1>
                                <span className='text-[14px] sm:text-[16px] text-[#3f4941] font-medium'>Review your presence across all enrolled courses.</span>
                            </div>
                        ...
                        <div className='flex flex-col lg:flex-row gap-4 sm:gap-6 lg:gap-[40px] mt-6'>
                            ...
                            <div className='w-full lg:w-[65%] bg-[#ffffff] p-4 border-1 border-[#bfc9bf] rounded-md'>
                                <div className='flex flex-col sm:flex-row justify-between sm:items-center gap-2 mb-4'>
                                    <div>
                                        <h2 className='text-[18px] sm:text-[20px] font-bold leading-tight'>Overall Attendance</h2>
                                        <span className='text-[12px] text-[#3f4941]'>Requirement: 75% for eligibility</span>
                                    </div>
                            ...
                            <div className='bg-[#0a643a] w-full lg:w-[35%] p-4 rounded-xl'>
                                <span className='font-medium text-[#e2e9ec] text-sm sm:text-base'>Total Lectures</span>
